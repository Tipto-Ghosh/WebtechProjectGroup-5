document.addEventListener("DOMContentLoaded", function () {
	...
	var quizStatus = page.getAttribute("data-quiz-status") || "draft";
	...
	var questionCountValue = document.getElementById("question_count_value");
	...
	function syncCard(card, questionData) {
	...
	function updateQuizState(quiz) {
		if (!quiz) {
			return;
		}

		quizId = String(quiz.id || quizId || "0");
		quizStatus = quiz.status || quizStatus || "draft";
		page.setAttribute("data-quiz-id", quizId);
		page.setAttribute("data-quiz-status", quizStatus);

		if (quizStatusBadge) {
			quizStatusBadge.textContent = quizStatus === "published" ? "Published" : "Draft";
			quizStatusBadge.className = quizStatus === "published" ? "badge badge-published" : "badge badge-draft";
		}

		if (quizMetaText) {
			quizMetaText.textContent = (quiz.time_limit_minutes || 0) + " min · " + (quizStatus === "published" ? "Published" : "Draft") + " · " + (quiz.question_count || 0) + " question" + ((quiz.question_count || 0) === 1 ? "" : "s");
		}
		...
		if (questionCountValue) {
			questionCountValue.textContent = (quiz.question_count || 0) + " questions";
		}

		if (publishButton) {
			publishButton.textContent = quizStatus === "published" ? "Unpublish Quiz" : "Publish Quiz";
		}
	}

	function renderView(card, questionData) {
		var optionsHtml = "";
		labels().forEach(function (label) {
			var optionText = questionData.options[label] || "";
			optionsHtml += '<span class="option_badge ' + (label === questionData.correct_option ? 'option_correct' : '') + '">' + label + ': ' + escapeHtml(optionText) + '</span>';
		});
	...
	function renderEdit(card, questionData) {
		var optionsHtml = "";
		labels().forEach(function (label) {
			var optionText = questionData.options[label] || "";
			optionsHtml += '' +
	...
	function updateIndexes() {
		...
		if (questionCountValue) {
			questionCountValue.textContent = cards.length + " questions";
		}
	}
	...
	if (questionForm) {
		questionForm.addEventListener("submit", function (event) {
			...
			var payload = {
				...
				correct_option: selected ? selected.value : "",
	...
	if (publishButton) {
		publishButton.addEventListener("click", function () {
			if (!quizId || quizId === "0") {
				alert("Save the quiz first before publishing.");
				return;
			}
	...
	if (questionsList) {
		questionsList.addEventListener("click", function (event) {
			...
			if (action === "edit") {
				renderEdit(card, cardData);
				return;
			}

			if (action === "cancel") {
				renderView(card, cardData);
				return;
			}
			...
			if (action === "save") {
				...
				optionFields.forEach(function (field) {
					options[field.getAttribute("data-option-key")] = field.value;
				});
				...
				request("PATCH", {
					...
					question_text: textField ? textField.value : cardData.question_text,
					marks: marksField ? marksField.value : cardData.marks,
					correct_option: correctField ? correctField.value : cardData.correct_option,
					option_a: options.A,
					option_b: options.B,
					option_c: options.C,
					option_d: options.D
					...
					if (response.question) {
						...
						updated.index = cardData.index;
						syncCard(card, updated);
						renderView(card, updated);
					}
